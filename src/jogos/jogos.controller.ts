...
export class jogosController {
  constructor(private readonly jogosService: jogosService, private caslAbilityFactory: CaslAbilityFactory,) {}
  ...
  update(@Param('id') id: string, @Body() dto: UpdateJogosDto, @LoggedUser() user: User ){
    const ability = this.caslAbilityFactory.createForUser(user);
    const isAllowed = ability.can(Action.Create, user);
    if (!isAllowed) {
      throw new ForbiddenException('Somente admnistradores!');
